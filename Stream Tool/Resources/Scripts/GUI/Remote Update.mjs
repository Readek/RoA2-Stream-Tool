...
export async function updateGUI(data) {
    ...
    for (let i = 0; i < data.player.length; i++) {
        ...
        // player character, skin, palette
        if (data.player[i].char != players[i].char
        || data.player[i].skin != players[i].skin.name
        || data.player[i].palette != players[i].skin.palette) {

            await players[i].charChange(data.player[i].char, true);
            await players[i].skinChange(players[i].findSkin(data.player[i].skin, true));
            await players[i].paletteChange(data.player[i].palette);
