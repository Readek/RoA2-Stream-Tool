...
export async function swapPlayers() {
    ...
    for (let i = 0; i < players.length; i+=2) {
        ...
        const tempP1Pal = players[i].palette;
        const tempP2Pal = players[i+1].palette;

        // update the stuff
        await players[i].charChange(tempP2Char, true);
        await players[i+1].charChange(tempP1Char, true);
        await players[i].skinChange(tempP2Skin, true);
        await players[i+1].skinChange(tempP1Skin, true);
        players[i].paletteChange(tempP2Pal);
        players[i+1].paletteChange(tempP1Pal);
