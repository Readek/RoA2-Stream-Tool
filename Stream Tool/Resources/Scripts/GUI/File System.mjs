...
export async function getCharacterList() {
    ...
    if (inside.electron) {
        ...
        saveJson(`/Remote/Character List`, characterList);

        return characterList;

    } else {

        return await getJson(`${stPath.text}/Remote/Character List`);
...
export async function getPresetList(folderName) {
    ...
    if (inside.electron) {
        ...
        // save for remote gui
        saveJson(`/Remote/${folderName}`, jsonList);

        return jsonList;

    } else {

        return await getJson(`${stPath.text}/Remote/${folderName}`);
...
export async function getPluginList() {
    ...
    if (inside.electron) {
        ...
        // save for remote gui
        saveJson(`/Remote/Plugin List`, files);

        return files;

    } else {

        return await getJson(`${stPath.text}/Remote/Plugin List`);
