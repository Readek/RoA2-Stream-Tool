...
import { getCharImage } from "../GetImage.mjs";
...
class PlayerFinder extends Finder {
                ...
                for (let i = 0; i < preset.characters.length; i++) {
                    ...
                    if (charFinder.isCharOnList(preset.characters[i].character)) {
                        ...
                        const pData = {
                            ...
                            palette : preset.characters[i].palette
                        }
    ...
    async loadFinderImgs(skinImgs) {
        ...
        for (let i = 0; i < skinImgs.length; i++) {
            ...
            const finalSrc = await getCharImage(
                skinImgs[i].char,
                skin,
                skinImgs[i].palette,
                "Skins",
    ...
    async #entryClick(pData, player) {
        ...
        // search for all skins for name matches
        player.skinChange(player.findSkin(pData.skin), true);
        // dont forget to palette swap
        if (player.paletteChange) { // lazy fix for bracket players
            player.paletteChange(pData.palette);
        }
